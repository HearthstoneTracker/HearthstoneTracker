// --------------------------------------------------------------------------------------------------------------------
// <copyright file="DXHookD3D9.cs" company="">
//   
// </copyright>
// <summary>
//   The dx hook d 3 d 9.
    ...
    internal class DXHookD3D9 : BaseDXHook
    {
        ...
        private void HandleCaptureRequest(Device device)
        {
            try
            {
                ...
                if (this.previousRequestId != null || this.lastRequestId != null)
                {
        ...
        private void RetrieveImageDataThread()
        {
            while (true)
            {
                ...
                var requestId = this.lastRequestId.Value;
        ...
        private void SetupSurfaces(Device device)
        {
            try
            {
                for (int i = 0; i < BUFFERS; i++)
                {
                    this.surfaces[i] = Surface.CreateOffscreenPlain(device, this.width, this.height, this.format, Pool.SystemMemory);
                    var lockedRect = this.surfaces[i].LockRectangle(LockFlags.ReadOnly);
